...
class CEMetricCalculator:
    ...
    def calculate_ce_metric(self, all_functions, composition_generator, N_samples=1000, test_functions=None, final_output=True):
        ...
        for idx, func in enumerate(all_functions):
            ...
            for s1, s2 in zip(input_strings_1, input_strings_2):
                if final_output:
                    ...
                    step_by_step_outputs = composition_generator.apply_function_composition(func, s1, s2)
                    
                    out = [o[:self.max_seq_len] for o in step_by_step_outputs]
                outputs.append(out)
        ...
        for i in range(n_functions):
            for j in range(i + 1, n_functions):
                ...
                if final_output:
                    ...
                    matches = [o1 == o2 for o1, o2 in zip(all_outputs[i], all_outputs[j])]
                    # get mean of matches
                    matches = np.mean(matches)
...
def main():
    ...
    parser.add_argument(
        "--final_output", type=bool, default=False, help="whether to use final output or all outputs"
    )
