...
def sanity_checks(cfg, loader):
    ...
    cfg.net.context_size = seq_len
    cfg.net.vocab_size = vocab_len
...
# Evaluate
@torch.no_grad()
def evaluate(net, evalLoaders, pad_pos, device_info):
    all_loss, all_acc = [], []
    ...
    for idx, split in enumerate(("train", "test")):
        ...
        for dat, targets in loader:
            ...
            with torch.amp.autocast(device_type=device, dtype=dt):
                ...
                logits = net(dat)[:, pad_pos:]
                targets = targets[:, pad_pos:]
    ...
    for idx, split in enumerate(("train", "test")):
        ...
        for dat, targets in loader:
            ...
            with torch.amp.autocast(device_type=device, dtype=dt):
                ...
                dat = dat[:, : seq_info["prompt_pos_end"]]
                output = generate(net, dat, seq_info["new_len"], lstm)
...
def log_eval(it, lr, eval_info, eval_info2=None, logger=None):
    ...
    if eval_info2 is not None:
        ...
        logger.info(
            f"loss (train/test): {eval_info2['train_loss']:.4f}/{eval_info2['test_loss']:.4f}"
        )
