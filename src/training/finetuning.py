...
class FineTuner:
    ...
    def compute_loss(self, dat, targets, device_info):
    ...
    def update_cosine_warmup_lr(self, it, cfg, total_steps):
        it += 1
        ...
        for epoch in range(self.cfg.epochs):
            for dat, targets in train_loader:
                ...
                # if it is time to log the training loss
                    ...
                    self.log_train(train_loss)
                # update the learning rate
                it, lr = self.update_cosine_warmup_lr(it, self.cfg.optimizer, self.optimizer, total_steps)
                # zero the gradients
                self.optimizer.zero_grad(set_to_none=True)
                # move the data to the device
                dat, targets = self.move_to_device(dat, targets, self.cfg.device)
                # compute the loss
                loss = self.compute_loss(dat, targets)
                # backward the loss
                loss.backward()
                # clip the gradients
    ...
    def evaluate(self, evalLoaders, device_info, sep_pos):
        ...
        device, dt = device_info
        self.model.eval()
        for idx, split in enumerate(("train", "train_heldout")):
        ...
        info = {
            ...
            "train_acc": all_acc[0],
            "train_heldout_loss": all_loss[1],
            "train_heldout_acc": all_acc[1],
        }
        self.model.train()
        return info

    def log_eval(self, it, lr, eval_info):
        self.logger.info(f"Iteration {it}, Learning Rate {lr:.6f}")
        self.logger.info(f"Train Loss: {eval_info['train_loss']:.6f}, Train Acc: {eval_info['train_acc']:.6f}")
        self.logger.info(f"Test Loss: {eval_info['test_loss']:.6f}, Test Acc: {eval_info['test_acc']:.6f}")
