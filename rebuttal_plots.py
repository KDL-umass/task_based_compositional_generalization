...
def _scatter_panel(ax, rel_plot_data, font_size, title):
    ...
    color_legend = ax.legend(
        ...
        fontsize=font_size * 0.55, frameon=False,
    )
    ...
    ax.legend(handles=size_handles, loc="upper center",
              ...
              fontsize=font_size * 0.45, frameon=False)
...
def plot_combined(train_results, val_results, test_results, leakage_list,
    ...
    1×4 figure:
      ...
      [1] Eq. class scatter – nh6_nl3
      [2] Eq. class scatter – nh12_nl12
      [3] Eq. class scatter – gemma1
    ...
    """
    # split style/marker encoding – shared across all model/pos combinations
    SPLIT_STYLE = {
    ...
    fig, axs = plt.subplots(1, 4, figsize=(90, 18))
    ...
    # Legend 1 – colors (model + pos type)
    color_entries = []
    for model_name in MODELS:
        abs_label = MODEL_LABELS[model_name] if model_name == "gemma1" \
                    else f"{MODEL_LABELS[model_name]} (Abs)"
    ...
    color_legend = axs[0].legend(
        handles=color_entries, loc="upper center",
        bbox_to_anchor=(0.5, -0.22), ncol=2,
        fontsize=font_size * 0.5, frameon=False, title="Model / pos. embed."
    )

    # Legend 2 – split styles (train / val / test)
    split_entries = [
    ...
    axs[0].add_artist(color_legend)
    axs[0].legend(
        handles=split_entries, loc="upper center",
        bbox_to_anchor=(0.5, -0.36), ncol=3,
        fontsize=font_size * 0.5, frameon=False, title="Split"
    )

    # --- Panels 1-3: per-model cluster scatter ---
    for i, model_name in enumerate(MODELS):
        ax = axs[i + 1]
        data = cluster_data_per_model.get(model_name, [])
        if not data:
    ...
    out_path = os.path.join(
        save_dir, "leakage_experiment_diverse_k6_equivalence_class_disjoint7.pdf"
    )
